...
TEST_CASE("Check the correctness of subtraction (operator -)") {
    ...
    SUBCASE("on ComplexVariable")
    {
        ...
        CHECK_LE(abs(solve(2i-3i-2i-y-y-11-y-y-y-10i-y-3i == 7-y-y-19i-y-0i-y).imag()-(3/2)), EP);//-9+(3/2)i
        CHECK_LE(abs(solve(0i-1i-5i-y-3-2i-61-3i == 7i-3i-y-y-3i-y+1).real()-(63/2)), EP);//(63/2)-6i
        CHECK_LE(abs(solve(0i-1i-5i-y-3-2i-61-3i == 7i-3i-y-y-3i-y+1).imag()+6), EP);//(63/2)-6i
...
TEST_CASE("Check the correctness of multiplication (operator *)") {
    ...
    SUBCASE("on ComplexVariable")
    {
        ...
        CHECK_LE(abs(solve(4*y == 2i).imag()-(1/2)), EP);//0+(1/2)i
        CHECK_LE(abs(solve(4*y*0i*4*5i == y).real()-0), EP);//0+0i
        CHECK_LE(abs(solve(4*y*0i*4*5i == y).real()-0), EP);//0+0i
        //CHECK_LE(abs(solve(y*1i*1i == y).real()+1, EP);//-1+0i
        //CHECK_LE(abs(solve(y*1i*1i == y).imag()-0, EP);//-1+0i
    }
...
TEST_CASE("Check the correctness of power (operator ^)") {
    ...
    SUBCASE("on RealVariable")
    {
        ...
        CHECK(((abs(solve((x^2) == 6)-3) <= EP) || (abs(solve((x^2) == 6)+3) <= EP)));//3 or -3
        CHECK(((abs(solve((x^2) == 64)-8) <= EP) || (abs(solve((x^2) == 64)+8) <= EP)));//8 or -8
        CHECK_LE(abs(solve((x^2) == 0)-0), EP);//0
        CHECK_LE(abs(solve(0 == (x^2))-0), EP);//0
        CHECK(((abs(solve(2^2^2^3 == x^2)-64) <= EP) || (abs(solve(2^2^2^3 == x^2)+64) <= EP)));//64 or -64
        CHECK(((abs(solve((((x^1)^1)^2) == 4)-2) <= EP) || (abs(solve((((x^1)^1)^2) == 4)+2) <= EP)));//2 or -2
        CHECK_LE(abs(solve(x^2 == x)-1), EP);//1
        CHECK_LE(abs(solve(x^2 == x*-1)+1), EP);//-1
    }
    ...
    SUBCASE("on ComplexVariable")
    {
        ...
        CHECK(((abs(solve((x^2) == 6).real()-3) <= EP) || (abs(solve((x^2) == 6).real()+3) <= EP)));//3+0i or -3+0i
        ...
        CHECK(((abs(solve(2^2^2^3 == x^2).real()-64) <= EP) || (abs(solve(2^2^2^3 == x^2).real()+64) <= EP)));//64+0i or -64+0i
        CHECK(((abs(solve((((x^1)^1)^2) == 4).real()-2) <= EP) || (abs(solve((((x^1)^1)^2) == 4).real()+2) <= EP)));//2+0i or -2+0i
        CHECK_LE(abs(solve(x^2 == x).real()-0), EP);//0+0i
        CHECK_LE(abs(solve(x^2 == x).real()-1), EP);//1+0i
        CHECK_LE(abs(solve(x^2 == x*-1).real()+1), EP);//-1+0i
    }
...
TEST_CASE("Check the correctness of equation 1") {
    ...
    {
        RealVariable x;
        CHECK_LE(abs(solve(x+x-2+3*x == 4*x/2)+1), EP);//-1
    ...
    {
        ComplexVariable x;
        CHECK_LE(abs(solve(x+x-2+3*x == 4*x/2).real()+1), EP);//-1+0i
...
TEST_CASE("Check the correctness of equation 2") {
    ...
    SUBCASE("on RealVariable")
    {
        ...
        CHECK(((abs(solve(6*x^2+6*x+4 == 4*x^2+2*x+4)+2) <= EP) || (abs(solve(6*x^2+6*x+4 == 4*x^2+2*x+4)+0) <= EP)));//-2 or 0
        CHECK(((abs(solve(3+2*x*x+x*x+10*x+12*x+3+10+2+39 == x*x-3)+6) <= EP) || (abs(solve(3+2*x*x+x*x+10*x+12*x+3+10+2+39 == x*x-3)+5) <= EP)));//-5 or -6
        CHECK(((abs(solve((x+2)*(x-2) == 0)+6) <= EP) || (abs(solve((x+2)*(x-2) == 0)+5) <= EP)));//2 or -2
        CHECK(((abs(solve((x+2)*(x+2) == 0)+6) <= EP) || (abs(solve((x+2)*(x+2) == 0)+5) <= EP)));//x^2+2*2*x+2^2 - > -2
    }
    ...
    SUBCASE("on ComplexVariable")
    {
        ...
        CHECK(((abs(solve(6*x^2+6*x+4 == 4*x^2+2*x+4).real()+2) <= EP) || (abs(solve(6*x^2+6*x+4 == 4*x^2+2*x+4).real()+0) <= EP)));//-2+0i or 0+0i
        ...
        CHECK(((abs(solve((x+2)*(x-2) == 0).real()+6) <= EP) || (abs(solve((x+2)*(x-2) == 0).real()+5) <= EP)));//2+0i or -2+0i
        CHECK(((abs(solve((x+2)*(x+2) == 0).real()+6) <= EP) || (abs(solve((x+2)*(x+2) == 0).real()+5) <= EP)));//x^2+2*2*x+2^2 - > -2+0i
        CHECK_LE(abs(solve(x+5i == 2*x+3i+10).real()+10), EP);//-10+2i
        CHECK_LE(abs(solve(1i^2+x*4-51 == 0).real()-13), EP);//13+0i
